...
const toolsTitle = "font-serif text-2xl text-plum-700 dark:text-foreground md:text-4xl md:leading-tight";

export function ToolsGrid() {
  return (
    <Section tone="parchment" aria-labelledby="tools-heading" className="dark:bg-[#0E0A14]">
      <header className="mb-12 text-center md:mb-16">
        <div className="flex flex-col items-center gap-3 md:gap-4">
          <div className="flex justify-center">
            <EyebrowBadge
              className="dark:bg-transparent dark:text-chartreuse-500 dark:ring-1 dark:ring-chartreuse-500/20"
            >
            ...
            >
              <div className="flex h-full flex-col p-6">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-chartreuse-300 dark:bg-chartreuse-500/15">
                  <tool.icon className="h-6 w-6 text-plum-700 dark:text-chartreuse-500" aria-hidden />
                </div>
                <div className="mt-4 flex items-start justify-between gap-3">
                  <h3 className="font-serif text-title text-plum-700 dark:text-chartreuse-500">
                    {tool.title}
                  </h3>
                  {tool.href.startsWith("/donate") || tool.href.startsWith("/causes") || tool.href.startsWith("/vote") ? (
